...
io.on('connection', (socket) => {
  ...
  socket.on('endGame', ({ lobbyCode }) => {
    const lobby = lobbies[lobbyCode];
    // Kein check auf lobby.started
    if (!lobby || lobby.ownerId !== socket.id) {
      console.log('🔒 EndGame verweigert:', lobbyCode, socket.id);
      return;
    }
    const spyNames = lobby.currentSpies.map(p => p.name);
    console.log('🏁 Spiel beendet, Spione:', spyNames);
  ...
  function startNewGame(lobbyCode, spyCount, category) {
    ...
    // gleiche Wortkategorien wie zuvor...
